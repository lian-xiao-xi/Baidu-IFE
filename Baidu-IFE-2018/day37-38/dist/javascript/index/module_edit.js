define(["require", "exports", "../common/common"], function (require, exports, common_1) {
     ...
     */
    function saveData() {
        var tableEle = common_1.getEle(document, '.data-table')[0];
        var thead_th = common_1.getEle(tableEle, '.data-thead .thead-tr th');
        var tbody_tr = common_1.getEle(tableEle, '.data-tbody .tbody-tr');
        ...
        for (var i = 0; i < tbody_tr.length; i++) {
            ...
            for (var j = 0; j < thead_th.length; j++) {
                var thItem = thead_th[j];
                switch (thItem.getAttribute('data-type')) {
                    ...
                    default:
                        if (thItem.getAttribute('data-month') !== null) {
                            var val = common_1.getEle(common_1.getEle(item, 'td')[j], '.show')[0].textContent.trim();
    ...
    function editData() {
        ...
        // 确认按钮响应的事件
        function sure(td) {
            var inputVal = common_1.getEle(td, 'input.edit-input')[0].value;
        ...
        var tbody = common_1.getEle(document, '.data-table .data-tbody')[0];
        // 当鼠标点击某个数字的单元格时，这个数字进入编辑状态，单元格内容变成一个输入框，输入框右边是取消和确定
        tbody.addEventListener('click', function (event) {
            var targetEle = event.target;
            var parent = targetEle.parentElement;
            if (targetEle.getAttribute('data-id') === 'edit') {
                var editInput = common_1.getEle(parent, 'input.edit-input')[0];
                var showDataEle = common_1.getEle(parent, '.show')[0];
                // 如果已经点击过其他td了，就先把那个取消，再继续
                if (document.getElementById('cancel')) {
                    // let cancel = document.getElementById('cancel');
                    var parentTd = document.getElementById('cancel').parentElement;
                    cancel(parentTd);
                }
                var tdEditEle = "<button id=\"cancel\" style=\"margin-right: 10px;\">\u53D6\u6D88</button><button id=\"sure\">\u786E\u5B9A</button>";
        ...
        function cancelAnather(domFir, target) {
            ...
            if (!(domFir == target || domFir.contains(target))) {
                var escEle = document.getElementById('cancel');
                if (escEle) {
                    var parent = escEle.parentElement;
                    cancel(parent);
                    console.log('取消取消取消');
                }
            }
