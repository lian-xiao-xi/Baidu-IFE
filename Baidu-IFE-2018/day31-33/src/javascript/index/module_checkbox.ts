 ...
 */
export function bindClickHandle(wapperId: string) {
  let wrapperEle: Element = document.getElementById(wapperId);
  let elesArr: HTMLInputElement[] = Array.prototype.slice.call(getEle(wrapperEle, 'input[data-check="item"]'));
  let checkallEle: HTMLInputElement = wrapperEle.querySelector('input[data-check="checkall"]')
...
function checkedLen(elesArr: HTMLInputElement[]): number {
  ...
  elesArr.forEach((item: HTMLInputElement) => {
